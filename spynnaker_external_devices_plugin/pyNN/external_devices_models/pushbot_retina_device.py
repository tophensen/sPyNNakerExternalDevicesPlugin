...
# Named tuple bundling together configuration elements of a pushbot resolution config
PushBotRetinaResolutionConfig = namedtuple("PushBotRetinaResolution", 
                                           ["pixels", "enable_command", "coordinate_bits"])
...
class PushBotRetinaDevice(AbstractVirtualVertex,
    ...
    def __init__(self, virtual_chip_x, virtual_chip_y,
        ...
        if not isinstance(polarity, PushBotRetinaPolarity):
            raise exceptions.ConfigurationException("Pushbot retina polarity should be one of those defined in Polarity enumeration")
        if not isinstance(resolution, PushBotRetinaResolution):
            raise exceptions.ConfigurationException("Pushbot retina resolution should be one of those defined in Resolution enumeration")
